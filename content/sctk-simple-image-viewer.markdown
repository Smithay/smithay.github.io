...
post aims at givin a general feeling of how this toolkit can be used, and in general how
...
a client app will need to connect to an unix socket on which the wayland server is listening,
and then, via this socket connexion the client will send requests to the server ("I want to
...
and all messages regarding pointer input will be receive through this object.

As said previously, all messages are attached to an object, and objects are created or destroyed
by messages. Thus all objects are created by messages of an other object, which is then its
parent. We can this way see the hierarchy of all objects as a tree. At its root exists the
`wl_display`, this object is automatically created when the connexion is established, and
represents the connexion itself. From the display a client will instanciate the `wl_registry`,
...
global objects. These objects can be instanciated from the registry, and are global in the
sense that they represent an underlying resource, and no matter how many times you create
an instance of a global, all these instances represent the same underlying resources.

These globals are of two kinds:
...
- Those that represent a ressource accessible to the compositor. Typically a peripheral (be it
  physical or logical). These can exist more than one time, for example each monitor plugged to
  the computer will appear in the registry as a `wl_output` global.

From these globals, the client can create more object (each global defining a subtree of 
objects) and send requests and receive events through these objects. All wayland interactions
are done this way.
...
notably includes the `wl_diplay` and `wl_registry` interfaces) that all wayland server is
...
The first part or our app is thus loading this image. This is in no way Wayand-specific,
...
as argument, and wraps it, processing its events, and automatically binding the most classic
...
fundamental building blocks of a wayland app. It describe some rectangle of pixels to which
you can attach a `wl_buffer` defining its contents. A wayland app is thus built on surfaces:
...
- They can be defined relative to a parent surface (known as a "subsurface"), this can
  typically be usefull if different parts of your UI are drawn by different means. For example
  a video play could use GPU-accelerated graphism to decode and display the contents of the
  video, and doing the rest of its UI with CPU drawings. Subsurfaces are also what SCTK uses
  to draw the decorations of your windows, as we'll see later.
- Input information is given relative to surfaces. A wayland app does not know absolute location
...
- it processes most of the shell interaction for use (maximizing/minimizing, etc..)

But it cannot do everything, and requires us to provide a callback to which some
events will be given, for us to process ourselves.
...
a function that redraw our window for a given size, resizing the loaded image if
necessary.

Defining the contents of a surfaces is done in wayland by first drawing the contents
ourselves, then creating a `wl_buffer` referencing this content, then attaching the
...
We now will write an utility function that will do the drawing itself. Taking a mutable
...
As our app has very little room for use input (we handle nothing except the window being 
...
Thanks for following me to the end, and I hope I managed to give a good general
